...
export default function PipelineColumn({ stage, deals, showOwner = false, onReassign, selectable = false, selected = [], onSelect }) {
  ...
  return <section ref={setNodeRef} className={`pipeline-column ${isOver ? 'is-over' : ''}`}>
    ...
    <div className="pipeline-column-body">{deals.length === 0 ? <div className="pipeline-empty"><span>+</span><p>Drop a deal here</p></div> : deals.map((deal) => <DealCard key={deal.id} deal={deal} showOwner={showOwner} onReassign={onReassign} selectable={selectable} selected={selected.includes(deal.id)} onSelect={onSelect} />)}</div>
  </section>
}
